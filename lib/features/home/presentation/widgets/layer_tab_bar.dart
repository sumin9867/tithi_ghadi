...
class LayerTabBar extends StatelessWidget {
  ...
  String? _nameFor(String layer) {
    switch (layer) {
      case 'nakshatra':
        return day.nakshatra.nameNp;
      case 'yoga':
        return day.yoga.nameNp;
      case 'karana':
        return day.karana.nameNp;
      default:
        return day.tithi.nameNp;
    }
  }
        ...
        children: [
          ...
          Row(
            children: topLayers.map((ly) {
              return Expanded(
                child: Padding(
                  padding: EdgeInsets.only(right: ly == 'tithi' ? 6 : 0),
                  child: _LayerCard(
                    layer: ly,
                    name: _nameFor(ly) ?? '',
          ...
          Row(
            children: bottomLayers.map((ly) {
              return Expanded(
                child: Padding(
                  padding: EdgeInsets.only(right: ly == 'yoga' ? 6 : 0),
                  child: _LayerCard(
                    layer: ly,
                    name: _nameFor(ly) ?? '',
                    isActive: activeLayer == ly,
...
const kLayerConfig = {
  'tithi': LayerConfig(
    ...
    detail: 'Ends at 08:14 PM',
  ),
  'nakshatra': LayerConfig(
    highlight: Color(0xFFe879a0),
    label: 'नक्षत्र',
    icon: Icons.auto_awesome,
    detail: '11h 05m remaining',
  ),
  'yoga': LayerConfig(
    highlight: Color(0xFF38bdf8),
    label: 'योग',
    icon: Icons.grain,
    detail: 'Next: Vishkumbha',
  ),
  'karana': LayerConfig(
    highlight: Color(0xFFa78bfa),
    label: 'करण',
    icon: Icons.straighten,
    detail: 'Active until 09:22 AM',
  ),
